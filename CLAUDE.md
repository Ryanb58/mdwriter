...
- `palette/` — Cmd+P fuzzy file palette, Cmd+K agent ask, Cmd+Shift+F vault content search (uses `cmdk`). `SearchMode.tsx` calls `ipc.searchVault` (Rust `commands/search.rs`) with a 180ms debounce. Search results set `pendingScroll: { path, line, matchText, occurrence }` in the store before selecting the file. Whichever editor mode is active consumes the target by walking matches in document order and stopping at `occurrence`: `RawEditor` uses `scrollViewToMatch` (CM doc walk); `BlockEditor` uses `findNthBlockMatch` then a DOM tree walker for the exact text Range. Both fire `flashHighlight` (a `.search-flash` overlay that fades over 500ms) at the match's viewport rect. The user's mode preference is respected — search never auto-flips.
