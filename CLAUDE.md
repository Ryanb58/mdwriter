...
- `palette/` — Cmd+P fuzzy file palette, Cmd+K agent ask, Cmd+Shift+F vault content search (uses `cmdk`). `SearchMode.tsx` calls `ipc.searchVault` (Rust `commands/search.rs`) with a 180ms debounce.
