...
mod tests {
    ...
    fn case_insensitive_by_default() {
        let dir = tempdir().unwrap();
        fs::write(dir.path().join("a.md"), "Hello World\n").unwrap();
        let r = search_vault_impl(dir.path().to_path_buf(), "world".into(), None).unwrap();
        assert_eq!(r.hits.len(), 1);
    }
    ...
    fn case_sensitive_when_requested() {
        ...
        let r = search_vault_impl(dir.path().to_path_buf(), "world".into(), Some(opts)).unwrap();
        assert_eq!(r.hits.len(), 1);
    ...
    fn empty_query_returns_nothing() {
        ...
        let r = search_vault_impl(dir.path().to_path_buf(), "   ".into(), None).unwrap();
    ...
    fn skips_non_markdown_files() {
        ...
        let r = search_vault_impl(dir.path().to_path_buf(), "needle".into(), None).unwrap();
        assert_eq!(r.hits.len(), 1);
    ...
    fn honors_gitignore_when_requested() {
        ...
        let r = search_vault_impl(dir.path().to_path_buf(), "needle".into(), Some(opts)).unwrap();
        assert_eq!(r.hits.len(), 1);
